...
# # === NEW ENV VARS (same as azure_avatar.py) ===
...
# # For ResourceID→BearerToken:
# token_provider = get_bearer_token_provider(DefaultAzureCredential(), "https://cognitiveservices.azure.com/.default")


# def calculate_energy(frame_data):
#     # Convert byte data (16‐bit PCM) into numpy array to estimate silence vs speech.
#     data = np.frombuffer(frame_data, dtype=np.int16)
#     energy = np.sum(data**2) / len(data)
#     return energy
...
#     def configure(self, voice: str):
#         """
#         Configure a “non‐avatar” TTS client with streaming ability.
#         If CUSTOM_TTS_VOICE_NAME is set, it overrides the voice passed in.
#         If CUSTOM_TTS_ENDPOINT_URL is set, we point our SpeechConfig endpoint there.
#         """
...
#             # CNV style: drop the "CNV:" prefix and flip to v1 for custom neural voice.
...
#         # === BUILD WEBSOCKET (or REST) ENDPOINT URL ===
#         if CUSTOM_TTS_ENDPOINT_URL:
#             base_ws = CUSTOM_TTS_ENDPOINT_URL
#             # If the URL doesn’t already have query params, append ?debug=3&trafficType=AzureSpeechRealtime
#             if "?" in base_ws:
#                 endpoint = base_ws
#             else:
#                 endpoint = f"{base_ws}?debug=3&trafficType=AzureSpeechRealtime"
#         else:
#             endpoint = (
#                 f"wss://{SPEECH_REGION}.{endpoint_prefix}.speech.microsoft.com"
#                 f"/cognitiveservices/websocket/{endpoint_version}?debug=3&trafficType=AzureSpeechRealtime"
#             )
...
#         # Build a small pool of SpeechSynthesizers to handle concurrent requests:
...
#     def text_to_speech(self, voice: str, speed: str = "medium") -> Tuple[InputTextStream, AioOutputStream]:
#         """
#         Returns (InputTextStream, AioOutputStream) so the caller can push text incrementally
#         and read PCM frames back. This is the streaming version.
#         """
#         start_time = time.time()
#         if self.endpoint_id:
#             # CNV (custom neural voice) does not support text‐streaming input; fall back to non‐streaming.
#             return self.text_to_speech_non_streaming(voice, speed)
...
#         async def read_from_data_stream():
#             leading_silence_skipped = False
#             # We skip leading silence so that “avatar lips” don’t move until real speech arrives.
#             silence_detection_frames_size = int(50 * 24000 * 2 / 1000)  # 50ms @ 24kHz × 2 bytes/sample
...
#                         continue
#                     frame_data = bytes(silence_detection_frames_size)
#                     read_len = await loop.run_in_executor(None, stream.read_data, frame_data)
#                     if read_len == 0:
#                         if stream.status != speechsdk.StreamStatus.AllData:
#                             logger.error(
...
#                     energy = await loop.run_in_executor(None, calculate_energy, frame_data)
#                     if energy < 500:
#                         # Still silence, keep skipping
#                         continue
#                     # We hit non‐silence: roll back position so we re‐read these samples:
#                     leading_silence_skipped = True
#                     stream.position = stream.position - silence_detection_frames_size

#                 chunk = bytes(2400 * 4)
#                 read_len = await loop.run_in_executor(None, stream.read_data, chunk)
#                 if read_len == 0:
#                     break
#                 if start_time < self.interruption_time:
#                     logger.warning("Interruption detected, stopping synthesis loop.")
#                     break
#                 aio_stream.write_data(chunk[:read_len])
...
#     def text_to_speech_non_streaming(self, voice: str, speed: str = "medium") -> Tuple[InputTextStream, AioOutputStream]:
#         """
#         Fallback for CNV (custom neural voice) which only supports one‐shot text (no streaming).
#         """
#         start_time = time.time()
#         logger.warning("Non‐streaming TTS synthesis is being used for CNV voice.")
#         logger.info(f"Synthesizing text with voice: {self.voice}")
...
#         async def read_from_data_stream():
#             # Gather all queued text, then call start_speaking_text(...) once:
#             inputs = []
#             async for chunk in input_stream:
#                 inputs.append(chunk)
...
#             leading_silence_skipped = False
#             silence_detection_frames_size = int(50 * 24000 * 2 / 1000)  # 50ms
...
#         logging.basicConfig(level=logging.INFO)
#         client = Client()
#         # If you want to test non-Avatar TTS, set CUSTOM_TTS_VOICE_NAME above:
#         client.configure("en-US-Andrew:DragonHDLatestNeural")
#         text_input, pcm_output = client.text_to_speech("en-US-Andrew:DragonHDLatestNeural")

#         async def write_text():
#             # Push a sample utterance in pieces, then close() so TTS plays it:
#             for segment in ["Hello, ", "هذا اختبار تونسية.", " كيف حالك؟"]:
#                 text_input.write(segment)
#             text_input.close()

#         async def read_audio():
#             with open("out.wav", "wb") as f:
#                 # Write minimal WAV header, then append raw PCM from pcm_output:
#                 audio_bytes = b""
#                 async for frame in pcm_output:
#                     audio_bytes += frame
#                 # Basic RIFF + WAVE + fmt chunk + data chunk:
#                 f.write(b"RIFF")
...
#                 f.write((24000).to_bytes(4, "little"))    # SampleRate = 24000
#                 f.write((24000 * 2).to_bytes(4, "little"))# ByteRate = SampleRate * BlockAlign
#                 f.write((2).to_bytes(2, "little"))        # BlockAlign = NumChannels * BitsPerSample/8
#                 f.write((16).to_bytes(2, "little"))       # BitsPerSample = 16
#                 f.write(b"data")
#                 f.write((len(audio_bytes)).to_bytes(4, "little"))
#                 f.write(audio_bytes)

#         await asyncio.gather(write_text(), read_audio())
